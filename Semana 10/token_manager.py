...
def decode_payload(token: str) -> dict:
    ...
    """
    try:
        partes = token.split('.')  # divide el token en tres partes separadas por puntos
        if len(partes) < 3: #si el token no tiene al menos tres partes, se lanza un error
            raise ValueError("Token mal formado. Le faltan partes.")
        
        payload_segment = partes[1] #toma la segunda parte del token (payload)
...
class TokenManager:
    ...
    def is_expiring_soon(self) -> bool:
        ...
        try:
            ...
            return time.time() + 10 >= exp #toma el tiempo actual y le suma 10 segundos, 
            # si es mayor o igual al tiempo de expiración del token, retorna True
        except Exception:
            return True
